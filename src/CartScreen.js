...
export default function CartScreen() {
  ...
  const handlePlaceOrder = () => {
    if (items.length === 0) {
      Alert.alert(t('cartEmpty'), t('addSomeProductsFirst'));
      return;
    }
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={clearCart} style={styles.headerRightBtn}>
          <Text style={styles.headerRightText}>{t('clear')}</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 120 + (insets?.bottom || 0) }]}>
        {items.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>{t('addProductsToCart')}</Text>
          </View>
        ) : (
          items.map((it) => (
            <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  itemPrice: { fontSize: 14, color: '#0e7c36', fontWeight: '700', marginTop: 4 },
  qtyRow: { flexDirection: 'row', alignItems: 'center' },
  qtyBtn: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#0e7c36', alignItems: 'center', justifyContent: 'center' },
  ...
  checkoutBtn: { backgroundColor: '#0e7c36', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10 },
